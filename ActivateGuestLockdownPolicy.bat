...
set "guest_policy=%script_dir%GuestLockdownPolicy"

if not exist "%guest_policy%" (
    echo Sorry the host policy is missing. Aborting.
    pause
    exit /b 1
)

xcopy /s /e /y "%guest_policy%\*" "%current_policy%"

gpupdate /force

echo Updated Current Policy to Guest GPO, Please restart to enact all changes
